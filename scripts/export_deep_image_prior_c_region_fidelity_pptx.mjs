#!/usr/bin/env node
/** Export Candidate C's region-first fidelity review as editable PowerPoint objects. */
...
function usage() {
  return [
    "Usage:",
    "  export_deep_image_prior_c_region_fidelity_pptx.mjs \\",
...
function addNativeShape(slide, shape) {
  ...
  return slide.shapes.add({
    geometry: shape.type === "ellipse" ? "ellipse" : "rect",
    name: shape.id,
...
async function readApprovedImage(module, assetDir) {
  const filePath = path.resolve(assetDir, module.asset);
  const assetRoot = `${path.resolve(assetDir)}${path.sep}`;
  if (!filePath.startsWith(assetRoot)) throw new Error(`${module.id} asset escapes --asset-dir`);
  await assertFile(filePath, `${module.id} exact raster atom`);
...
async function buildPresentation(spec, equationDir, assetDir, artifactModule) {
  ...
  if (counts.native_region_shapes < 500) throw new Error("At least 500 native region shapes are required");
  ...
  const notes = [
    ...
    "Candidate C is not embedded as a whole-slide image. Overlay and reference crops are not embedded.",
    "Two exact image atoms are independent, replaceable picture shapes; they are not pixel-editable or scientific evidence.",
    "All frames, generator layers, plots, connectors, arrowheads, and labels are native PowerPoint objects.",
    "Equations are locally rendered vector SVG objects whose target boxes preserve intrinsic viewBox ratios.",
    "This segmented review draft does not establish final scientific approval.",
    "[Sources]",
    "- source/selected_candidate_map.json — hash-bound eight-region visual direction map.",
  ...
  return { presentation, slide, counts, rasterRecords, equationRecords, PresentationFile };
}

async function exportArtifacts(spec, specBytes, equationDir, assetDir, artifactModule, outputDir) {
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "region-fidelity-pptx-"));
  try {
    const built = await buildPresentation(spec, equationDir, assetDir, artifactModule);
    const { presentation, slide, counts, rasterRecords, equationRecords, PresentationFile } = built;
    ...
    const report = {
      schema_version: "1.0",
      status: "AUTHORED_PENDING_INDEPENDENT_RENDER_QA",
      ...
      constraints: {
        ...
        native_region_shape_minimum_met: counts.native_region_shapes >= 500,
      ...
      verification_boundary: "Artifact authored; slide render inspection, overflow testing, and final scientific approval remain separate checks.",
    };
