#!/usr/bin/env python3
"""Structural tests for the approved Deep Image Prior editable delivery."""
...
DELIVERY_ROOT = EXAMPLE_ROOT / "editable_delivery"
...
class DeepImagePriorDeliveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_semantic_source_binds_D_layout_C_style_and_expected_topology(self) -> None:
        ...
        self.assertEqual(len(semantic["shapes"]), 214)
        self.assertEqual(len(semantic["text_objects"]), 10)
        self.assertEqual(len(semantic["equation_objects"]), 9)
        self.assertEqual(len(semantic["connectors"]), 8)
        ...
        self.assertEqual(
            hashes["layout_reference_D"],
            sha256_file(EXAMPLE_ROOT / "candidates" / "D-alternative-layout.png"),
        )
        self.assertEqual(
            hashes["visual_style_reference_C"],
            sha256_file(EXAMPLE_ROOT / "candidates" / "C-presentation.png"),
        )
        self.assertEqual(
            hashes["selection_approval"],
            sha256_file(EXAMPLE_ROOT / "candidate_selection.json"),
        )
        ...
        self.assertEqual(actual_edges, {
            ...
            "flow-comparison-loss": ("comparison-node", "loss-box"),
    ...
    def test_svg_pptx_drawio_and_pdf_have_native_verified_structure(self) -> None:
        ...
        self.assertEqual(pptx["native_connector_count"], 8)
        self.assertEqual(drawio["picture_standin_cells"], [])
        self.assertEqual(drawio["native_edge_count"], 8)
    ...
    def test_portable_validation_report_matches_checked_in_artifacts(self) -> None:
        ...
        self.assertEqual(report["validation_scope"], "Programmable structure only; not scientific correctness or Gate 3 approval.")
