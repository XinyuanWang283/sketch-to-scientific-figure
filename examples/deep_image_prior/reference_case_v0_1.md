...
The canonical review package for this case is [`editable_delivery_c_fidelity_v2/`](editable_delivery_c_fidelity_v2/README.md). This note explains its inputs, reconstruction, structural checks, and approval boundaries.
...
The canonical result starts from an approved eight-region map and preserves two narrowly authorized, independently replaceable synthetic image atoms. The remaining scientific structure is reconstructed as native/vector objects. The selected source, region map, raster exceptions, and delivered files are bound by hashes so the checks apply to this exact case.
